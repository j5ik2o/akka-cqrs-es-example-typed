...
public class RouteTestHelper extends RouteTest {
    ...
    private ActorTestKit createTestKit() {
        var setup = ActorSystemSetup.create(BootstrapSetup.create(additionalConfig)).withSetup(
                new JacksonObjectMapperProviderSetup(new JacksonObjectMapperFactory())
        );
        var system = akka.actor.typed.ActorSystem.create(
                Behaviors.empty(),
                "test-system",
                setup);
    ...
    @Override
    public TestRouteResult createTestRouteResultAsync(HttpRequest request, Future<RouteResult> result) {

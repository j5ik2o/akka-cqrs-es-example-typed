...
private[adceet_sbtecr] object AwsEcr extends Aws {
  ...
  def createRepository(
      ...
      repositoryLifecyclePolicyText: Option[String]
    ...
    try {
      val result = client.createRepository(
        new CreateRepositoryRequest()
          .withRepositoryName(repositoryName).withImageTagMutability(
            if (imageTagsMutable) ImageTagMutability.MUTABLE else ImageTagMutability.IMMUTABLE
          )
      )
  ...
  private def setPolicy(ecr: AmazonECR, repositoryName: String, repositoryPolicyText: String)(implicit
      logger: Logger
  ): Unit = {
  ...
  private def putLifecyclePolicy(ecr: AmazonECR, repositoryName: String, lifecyclePolicyText: String)(implicit
      logger: Logger
  ): Unit = {
  ...
  def dockerCredentials(region: Region, registryIds: Seq[String] = Seq.empty)(implicit
      logger: Logger
  ): (String, String) = {
    val request =
  ...
  private def ecr(region: Region)(implicit logger: Logger): AmazonECR = {
